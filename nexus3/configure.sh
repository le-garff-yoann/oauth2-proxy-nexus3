...
nexus3_configure_rut_realm()
{
    ...
    echo '{"name": "", "type": "", "content": ""}' | jq -cr \
        --arg name $script_name --arg type $script_type --arg content "$(cat $dir/$script_name.$script_type)" \
        '. + {name: $name, type: $type, content: $content}' > "$dir/$script_name.json"

    curl -sLku $2 -H 'Content-Type: application/json' -d @"$dir/$script_name.json" $script_endpoint/
