...
export \
    N3P_NEXUS3_ADMIN_USER=admin \
    N3P_NEXUS3_ORIGINAL_ADMIN_PASSWORD=$(docker-compose exec nexus3 cat /nexus-data/admin.password) \
    N3P_NEXUS3_ADMIN_PASSWORD=$(uuidgen)

echo "Change Nexus 3's default admin password."
nexus3_modify_user_password https://nexus3-direct.localhost \
    $N3P_NEXUS3_ADMIN_USER $N3P_NEXUS3_ORIGINAL_ADMIN_PASSWORD $N3P_NEXUS3_ADMIN_PASSWORD 1>/dev/null \
...
nexus3_configure_rut_realm https://nexus3-direct.localhost \
    "$N3P_NEXUS3_ADMIN_USER:$N3P_NEXUS3_ADMIN_PASSWORD" 1>/dev/null \
...
docker-compose up -d --build --force-recreate oauth2-proxy nexus3-proxy \
...
NEXUS3_ADMIN_PASSWORD=$N3P_NEXUS3_ADMIN_PASSWORD
EOF
